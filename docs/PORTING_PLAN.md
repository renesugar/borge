...
| 3 | 14 missing per-command options | §11.2, `option-coverage.sh` | measured, down from 111. On 2026-08-20 seven commands reached zero: `list`, `diff` (DIVERGENCES #48), `export-tar`, `import-tar` (#49), `prune` (#50), `extract` (#51) and `create` (#52). What remains: `recreate` 4 (row 4), `repo-create` 3 (row 2), `check` 2, `help` 2, and one each on `repo-delete`, `repo-list` and `key remove` |
